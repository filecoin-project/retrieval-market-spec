...
type BidSearchParams struct {
	// MinimumPricePerByteServed is the minimum amount paid for each retrieval on a per-byte basis
	MinimumPricePerByteServed abi.TokenAmount
	// MinimumPricePerRequestServed is the minimum amount paid for each retrieval on a per-byte basis
	MinimumPricePerRequestServed abi.TokenAmount
	// MaximumBandwidthBytes indicates the maximum expected transfer speed in bytes
	MaximumBandwidthBytes uint64
	// MinimumTimeToFirstByte specifies the minimum time to serve the first byte (miner latency)
...
// ContentBidIndexAPI provides a machinism for making and discovering bids for different
// content
type ContentBidIndexAPI interface {
	...
	// Search bids searches the content index to find bids that matche the search criteria
	SearchBids(ctx context.Context, params BidSearchParams) (<-chan ContentBid, error)
}
